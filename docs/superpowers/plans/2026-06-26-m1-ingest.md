...
**Scope:** MD/TXT + PDF extraction, chunking, persistence, ingest service + quality gate. **Out of scope:** prep-pack generation (→ M2), examiner/judge (→ M3), the import UI (→ M4), section-level (sub-chunk) evidence (later).
...
- [ ] **Step 3: Implement** — `src/lib/ingest/types.ts` (the contracts block above), then `src/lib/ingest/extract.ts`:
```ts
...
export function chunkParagraphs(paragraphs: Paragraph[], opts?: { maxChars?: number }): Chunk[] {
  ...
  for (const p of paragraphs) {
    const addLen = (buf.length ? SEP.length : 0) + p.text.length;
    const sectionChanged = buf.length > 0 && p.section !== section;
    if (buf.length > 0 && (sectionChanged || bufLen + addLen > maxChars)) {
      flush();
      cursor += SEP.length; // the SEP between the flushed chunk and this paragraph
    }
    if (buf.length === 0) { start = cursor; section = p.section; }
    buf.push(p);
    bufLen += addLen;
  }
  flush();
...
describe("insertThesisWithChunks", () => {
  ...
  it("is atomic — a bad chunk rolls back the whole thesis", () => {
    ...
    expect(() =>
      insertThesisWithChunks(db, {
        thesis: { id: "t1", title: "T", source_kind: "md" },
        // duplicate ord/id path: force a failure by passing a chunk with a null text (NOT NULL)
        chunks: [chunk(0, "ok"), { ord: 1, text: null as unknown as string, charStart: 0, charEnd: 0, hash: "h" }],
      }),
    ).toThrow();
