...
export function getThesisEvidence(db: DB, thesisId: string): { id: string; text: string }[] {
  return db.prepare("SELECT id, text FROM evidence_unit WHERE thesis_id=? ORDER BY id").all(thesisId) as { id: string; text: string }[];
}
...
export async function runPrepPackGeneration(
  ...
  try {
    ...
    for (const item of items) {
      ...
      try {
        bindPrepEvidence(db, prepId, item.evidence_unit_ids); // same-thesis enforced
        ...
        // cross-thesis / bad citation: mark the single item unsafe, keep the run going
